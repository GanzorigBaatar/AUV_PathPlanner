...
class ppCircleMarker(QgsMapCanvasItem):
    ...
    def paint(self, p, option, widget):  # option and widget belong to an overloaded method, but it only works like this
        ...
        if mapunits == QgsUnitTypes.DistanceMeters:
            myradius = round(self.radius / mapsettings.mapToPixel().mapUnitsPerPixel()  * (math.pi/2)) #ctx.scaleFactor()
            # QgsMessageLog.logMessage("Painting circle: mapunits={}, mapUnitsPerPixel={}, scaleFactor={}, scale={}".format(mapunits, mapsettings.mapToPixel().mapUnitsPerPixel(), ctx.scaleFactor(), self.canvas.scale()), 'Pathplanner', Qgis.INFO)
            ...
            # QgsMessageLog.logMessage("Painting circle: center={},{}, ul={},{}, radius={}".format(lon, lat, p1_lon, p1_lat, self.radius), 'Pathplanner', Qgis.INFO)
